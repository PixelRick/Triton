...
namespace triton {
  namespace ast {
    ...
    SharedAbstractNode newInstance(AbstractNode* node, bool unroll) {
      ...
      /* Cloning is done iteratively to avoid stack overflow */
      SharedAbstractNode ret;
      ...
      std::stack<WorkItem> worklist;
      worklist.push({node, nullptr, 0});

      SharedAbstractNode retNode;
      ...
      while (!worklist.empty()) {
        ...
        if (unroll) {
          while (node->getType() == REFERENCE_NODE) {
            node = reinterpret_cast<ReferenceNode*>(node)->getSymbolicExpression()->getAst().get();
          }
        }

        SharedAbstractNode newNode;
        switch (node->getType()) {
        ...
        if (newNode->getType() != VARIABLE_NODE) {
        ...
        /* Replace child in cloned parent, parent takes shared ownership of newNode */
        if (item.parent != nullptr) {
          item.parent->setChild(item.childIdx, newNode);
        }
        else {
          retNode = newNode;
        }

        /* Appends children to worklist */
        auto& children = newNode->getChildren();
        for (triton::uint32 idx = 0; idx < children.size(); ++idx) {
          worklist.push({children[idx].get(), newNode.get(), idx});
        }
      }
